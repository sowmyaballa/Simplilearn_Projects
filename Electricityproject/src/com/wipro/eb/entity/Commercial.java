...
public class Commercial extends Connection {
	...
	public float computeBill() {
		...
		if(finalamount >= 10000) {
			billamount = (float) (finalamount + (finalamount+(finalamount*0.09)));
		}
		else if((finalamount >= 5000) && (finalamount < 10000)) {
			billamount = (float) (finalamount + (finalamount+(finalamount*0.06)));
		}
		else {
			billamount = (float) (finalamount + (finalamount+(finalamount*0.02)));
		}
		return billamount;
		}
		}
